...
function MjpegConsumer(options) {
    ...
    this.bytesWritten = 0;
    this.forceSearchForStart = false;
}
...
MjpegConsumer.prototype._transform = function (chunk, encoding, done) {
    ...
    if (this.bytesWritten + chunk.length >= this.contentLength) {
    ...
    if (this.reading === false) {
        debug("Not currently reading finding start chunk");
    ...
    if (this.forceSearchForStart) {
        start = chunk.indexOf(soi);
        if (start === -1) {
            debug("Didn't find start in current frame");
            done();
            return;
        }
        debug("Found start of " + start + " in current frame");
        this.forceSearchForStart = false;
    }
